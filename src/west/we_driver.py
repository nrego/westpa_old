...
class WEDriver:
    ...
    def _split_walker(self, segment, m, bin):
        ...
        new_segments = []
        # NREGO - hack to add segment data
        newdata = {}
        for key in segment.data:
            newdata[key] = numpy.zeros_like(segment.data[key])
            newdata[key][0,:] = segment.data[key][0,:]
        # End NREGO mod
    ...
    def _merge_walkers(self, segments, cumul_weight, bin):
        ...
        # NREGO - hack to add segment data
        newdata = {}
        for key in gparent_seg.data:
            newdata[key] = numpy.zeros_like(gparent_seg.data[key])
            newdata[key][0] = gparent_seg.data[key][0]
        glom.data = newdata
        # End NREGO mod
    ...
    def construct_next(self):
        ...
        for ibin, _bin in enumerate(self.final_binning):
            for segment in _bin:
                ...
                for key in segment.data:
                    newdata[key] = numpy.zeros_like(segment.data[key])
                    newdata[key][0] = segment.data[key][-1]
                # End NREGO mod
